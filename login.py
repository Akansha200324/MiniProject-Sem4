    ...
    def forget_passwd(self):
        if self.cmb_quest.get()=="Select"or self.txt_answer.get()=="" or self.txt_new_passwd.get()=="":
            ...
            try:
                ...
                row=cur.fetchone()
                if row==None:
                    messagebox.showerror("Error","Please Select the Correct security question and answer",parent=self.root2)
                    ...
                    if len(self.txt_new_passwd.get())>=8:
                        ...
                        if(l>=1 and u>=1 and d>=1 and p>=1):
                            ...
                            messagebox.showerror("Error","Password should have 8 characters ,atleast one upper case letter,lower case letter,numeric value and special character '@','&','#','!','_'",parent=self.root2) 
                    else:
                        messagebox.showerror("Error","Password should have 8 characters ,atleast one upper case letter,lower case letter,numeric value and special character '@','&','#','!','_'",parent=self.root2)
        ...
        if self.txt_user.get()=="":
            ...
            try:
                ...
                #Title for Forget Interface
                    title=Label(self.root2,text="Forget Password",font=("Times New Roman",25),fg="red",bg="white").place(x=0,relwidth=1)
                #Contents in Forget Password Page
                    ...
                    new_passwd=Label(self.root2, text="New Password", font=("times new roman",15,"bold"),bg="white", fg="gray").place(x=50,y=260)
                    self.txt_new_passwd=Entry(self.root2, font=("times new roman", 15),bg="lightgray")
                    self.txt_new_passwd.place(x=50, y=290,width=250)

                    btn_change_passwd=Button(self.root2,text="Reset Password",command=self.forget_passwd,bg="Green",fg="White",font=("times new roman",15,"bold")).place(x=90,y=340)
    ...
    def login_function(self):
        if self.txt_pass.get()=="" or self.txt_user.get()=="" or self.txt_UserType.get()=="select":
            ...
            try:
                ...
                row=cur.fetchone()
                if row==None:
                    messagebox.showerror("Error","Invalid Username or Password or UserType",parent=self.root)
                    
                else:
                    if (self.txt_UserType.get()=="Student"):
                        messagebox.showinfo("Success",f"Welcome :- {self.txt_user.get()}",parent=self.root)
                        self.root.destroy()
